/*
 * ========================================================================
 * Ballsace 过检测逻辑 - 完整源码整合版
 * ========================================================================
 * 来源: ballsace.dylib / ballsa.dylib 全量反编译还原
 * 编译产物: ballsace_balls.dylib
 *
 * 模块组成:
 *   [1] common.h        - 公共头文件(平台检测/常量/日志宏/外部声明)
 *   [2] decrypt.c       - 字符串解密引擎(10种XOR变体还原)
 *                          (基础型XOR + 掩码型XOR + 乘法哈希取模 + 一次性标志位)
 *   [3] c2.c            - C2服务器连接与协议处理
 *                          (构造函数注册dyld回调 + 直连C2 + 握手协议 + 主循环)
 *   [4] il2cpp.c        - il2cpp API加载器 + 游戏作弊功能引擎
 *                          (26个API函数指针 + 反射查找 + 帧率解锁 + 灵敏度调节 + 硬编码偏移内存读写)
 *   [5] vm_patch.c      - vm_protect段补丁引擎
 *                          (遍历load commands定位段 -> 改保护属性 -> memcpy -> icache invalidate)
 *                          (签名破坏 + __DATA_CONST只读段补丁)
 *
 * 编译命令: make (在Makefile所在目录执行)
 * 目标平台: arm64 (iPhone)
 * ========================================================================
 */
...
/* LC_LINKEDIT_DATA 兜底 (旧SDK可能未定义) */
#ifndef LC_LINKEDIT_DATA
#define LC_LINKEDIT_DATA 0x2D
#endif
...
/* sys_icache_invalidate 前向声明 (某些SDK未导出) */
...
static uintptr_t g_real_connect_addr = 0;  /* 真实 connect 地址 (dlsym获取) */
...
static void save_real_connect(void)
{
    ...
    if (handle) {
        g_real_connect_addr = (uintptr_t)(intptr_t)dlsym(handle, "connect");
        LOGI("real connect addr saved: %p", (void *)(uintptr_t)g_real_connect_addr);
        dlclose(handle);
    }
}
...
/*
 * il2cpp.c - il2cpp API加载器 + 游戏作弊功能引擎
...
int apply_patch(void *target, size_t size, const void *patch_data)
{
    ...
    size_t page_size = page_size_val;

    uintptr_t page_start = (uintptr_t)target & ~(page_size - 1);
    mach_port_t task = mach_task_self();

    /* vm_protect: 改为 RWX (可读可写可执行) */
    kern_return_t kr = vm_protect(task, page_start, page_size, 0, VM_PROT_ALL);
    if (kr != KERN_SUCCESS) {
    ...
    /* vm_protect(task, page_start, page_size, 0, old_prot); */
