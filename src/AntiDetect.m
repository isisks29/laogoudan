...
static void save_real_connect(void)
{
    ...
    if (handle) {
        g_real_connect_addr = (int)dlsym(handle, "connect");
...
void unlock_fps(int target_fps)
{
    ...
    if (dm) {
        ...
        if (m) {
            int args = 1;
            g_runtime_invoke(m, dm, &args, NULL);
    ...
    if (ss) {
        ...
        if (m) {
            g_runtime_invoke(m, ss, &target_fps, NULL);
...
int patch_code_signature(struct mach_header_64 *header)
{
    ...
    if (segs.seg_data_const) {
        LOGI("vm_patch: __DATA_CONST found at 0x%lx, size=0x%lx",
