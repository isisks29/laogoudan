...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    _pressStarted = YES;
    self.isPressed = YES;
    if (self.onPress) self.onPress(YES);  // 立即触发，不再等150ms
}
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    // 仅调试模式下：移动超过5px视为拖动，取消宏并移动按钮
    if (!_isDragging && [GlobalConfig shared].debugMode && (fabs(dx) > 5 || fabs(dy) > 5)) {
        _isDragging = YES;
        if (_pressStarted) {
            _pressStarted = NO;
            self.isPressed = NO;
...
- (void)updateButtonPositions {
    ...
    self.shiliufenBtn.frame = CGRectMake(cfg.shiliufen.buttonX * screenSize.width - s16size/2,
                                          cfg.shiliufen.buttonY * screenSize.height - s16size/2, s16size, s16size);
    self.shiliufenBtn.hidden = !cfg.shiliufen.enabled;

    CGFloat tqsize = cfg.tuqiu.buttonSize * 2;
    self.tuqiuBtn.frame = CGRectMake(cfg.tuqiu.buttonX * screenSize.width - tqsize/2,
                                      cfg.tuqiu.buttonY * screenSize.height - tqsize/2, tqsize, tqsize);
    self.tuqiuBtn.hidden = !cfg.tuqiu.enabled;

    CGFloat s4size = cfg.sifen.buttonSize * 2;
    self.sifenBtn.frame = CGRectMake(cfg.sifen.buttonX * screenSize.width - s4size/2,
                                      cfg.sifen.buttonY * screenSize.height - s4size/2, s4size, s4size);
...
- (void)saveButtonPosition:(NSInteger)type center:(CGPoint)center {
    ...
    else { mc = cfg.sifen; mc.buttonX = relX; mc.buttonY = relY; cfg.sifen = mc; }
    [cfg save];
}
...
#pragma mark - 核心：IL2CPP 实例方法调用（需要GameCore实例，int参数）
...
- (void)startShiliufenLoop {
    ...
    self.shiliufenTimer = [NSTimer timerWithTimeInterval:interval target:self selector:@selector(shiliufenTick) userInfo:nil repeats:YES];
